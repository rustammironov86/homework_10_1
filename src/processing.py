def filter_by_state(data_operation: list[dict], state="EXECUTED") -> list[dict]:
    """Функция фильтрует выполненые и отмененые банковские операции"""
    return list(filter(lambda x: str(x["state"]) == state, data_operation))


def sort_by_date(date_list: list[dict], ascending=True) -> list[dict]:
